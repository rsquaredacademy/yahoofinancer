...
test_that("yf_get_market_stats works", {
  ...
  expect_warning(res4 <- yf_get_market_stats("FAIL"), "Failed to fetch data for ticker: FAIL")
  expect_equal(nrow(res4), 0)
})

test_that("yf_get_financials works with frequencies and statements", {
  MockTicker <- R6::R6Class("MockTicker",
    public = list(
      symbol = NULL,
      initialize = function(symbol) {
  ...
  res <- yf_get_financials("AAPL", "income", frequency = "annual")
  expect_s3_class(res, "tbl_df")
  expect_equal(nrow(res), 1)
  expect_equal(res$symbol, "AAPL")
  expect_equal(res$total_revenue, 1000)
  expect_equal(res$period_type, "12M")

  res2 <- yf_get_financials("AAPL", "balance-sheet", frequency = "quarterly")
  expect_equal(res2$total_assets, 5000)
  expect_equal(res2$period_type, "3M")

  res3 <- yf_get_financials(c("AAPL", "MSFT"), "cash-flow")
  expect_equal(nrow(res3), 2)
  expect_equal(res3$free_cash_flow, c(200, 200))

  expect_warning(res4 <- yf_get_financials(c("AAPL", "FAIL"), "income"), "Failed to fetch data for ticker: FAIL")
  expect_equal(nrow(res4), 1)

  expect_warning(res5 <- yf_get_financials("FAIL", "income"), "Failed to fetch data for ticker: FAIL")
