...
` ``output``$``price_card`` ``<-`` `[`renderText`](https://rdrr.io/pkg/shiny/man/renderPrint.html)`(``{`` `` ``s`` ``<-`` `[`req`](https://rdrr.io/pkg/shiny/man/req.html)`(``stats``(``)``)`` `` `[`req`](https://rdrr.io/pkg/shiny/man/req.html)`(`[`nrow`](https://rdrr.io/r/base/nrow.html)`(``s``)`` ``>`` ``0``)`` `` `[`sprintf`](https://rdrr.io/r/base/sprintf.html)`(``"%s · %s"``, ``s``$``symbol``, `[`dollar`](https://scales.r-lib.org/reference/dollar_format.html)`(``s``$``regular_market_price``)``)`` `` ``}``)`` `` `` ``output``$``change_card`` ``<-`` `[`renderUI`](https://rdrr.io/pkg/shiny/man/renderUI.html)`(``{`` `` ``s`` ``<-`` `[`req`](https://rdrr.io/pkg/shiny/man/req.html)`(``stats``(``)``)`` `` `[`req`](https://rdrr.io/pkg/shiny/man/req.html)`(`[`nrow`](https://rdrr.io/r/base/nrow.html)`(``s``)`` ``>`` ``0``)`` `` ``pct`` ``<-`` ``100`` ``*`` ``(``s``$``regular_market_price`` ``-`` ``s``$``previous_close``)`` ``/`` ``s``$``previous_close`` `` ``color`` ``<-`` ``if`` ``(``pct`` ``>=`` ``0``)`` ``"#2e7d32"`` ``else`` ``"#c62828"`` `` ``tags``$``span``(``style ``=`` `[`sprintf`](https://rdrr.io/r/base/sprintf.html)`(``"color:%s"``, ``color``)``,`` `` `[`sprintf`](https://rdrr.io/r/base/sprintf.html)`(``"%+.2f%% vs previous close"``, ``pct``)``)`` `` ``}``)`` `` `` ``output``$``range_card`` ``<-`` `[`renderText`](https://rdrr.io/pkg/shiny/man/renderPrint.html)`(``{`` `` ``s`` ``<-`` `[`req`](https://rdrr.io/pkg/shiny/man/req.html)`(``stats``(``)``)`` `` `[`req`](https://rdrr.io/pkg/shiny/man/req.html)`(`[`nrow`](https://rdrr.io/r/base/nrow.html)`(``s``)`` ``>`` ``0``)`` `` ``pos`` ``<-`` ``100`` ``*`` ``(``s``$``regular_market_price`` ``-`` ``s``$``fifty_two_week_low``)`` ``/`` `` ``(``s``$``fifty_two_week_high`` ``-`` ``s``$``fifty_two_week_low``)`` `` `[`sprintf`](https://rdrr.io/r/base/sprintf.html)`(`` `` ``"%s — %s\n(%s of 52-week range)"``,`` `` `[`dollar`](https://scales.r-lib.org/reference/dollar_format.html)`(``s``$``fifty_two_week_low``)``, `[`dollar`](https://scales.r-lib.org/reference/dollar_format.html)`(``s``$``fifty_two_week_high``)``,`` `` `[`percent`](https://scales.r-lib.org/reference/percent_format.html)`(``pos`` ``/`` ``100``, accuracy ``=`` ``1``)`` `` ``)`` `` ``}``)`` `` `` ``output``$``volume_card`` ``<-`` `[`renderText`](https://rdrr.io/pkg/shiny/man/renderPrint.html)`(``{`` `` ``s`` ``<-`` `[`req`](https://rdrr.io/pkg/shiny/man/req.html)`(``stats``(``)``)`` `` `[`req`](https://rdrr.io/pkg/shiny/man/req.html)`(`[`nrow`](https://rdrr.io/r/base/nrow.html)`(``s``)`` ``>`` ``0``)`` `` `[`label_number`](https://scales.r-lib.org/reference/label_number.html)`(``scale_cut ``=`` ``scales``::`[`cut_short_scale`](https://scales.r-lib.org/reference/number.html)`(``)``)``(``s``$``regular_market_volume``)`` `` ``}``)`

Two defensive details worth keeping:

- `req(nrow(s) > 0)` silently suspends rendering until a valid dataset
  arrives—since
  [`yf_get_market_stats()`](https://yahoofinancer.rsquaredacademy.com/reference/yf_get_market_stats.md)
  returns an empty tibble on network failure or invalid ticker, this
  check prevents empty rows from flashing errors into the UI.
- The percentage uses
...
- **Trending Tickers**: Automatically seed or update dropdown choices
  with currently trending securities via `get_trending("US")` or
  `get_trending("GB")`.
- **Auto-refresh**: Add
...
- **Multi-symbol comparison**: Fan out over several tickers at once with
  the `Tickers` class
  ([`?Tickers`](https://yahoofinancer.rsquaredacademy.com/reference/Tickers.md))—`Tickers$new(c("AAPL", "MSFT"))$get_history(period = "6mo")`
